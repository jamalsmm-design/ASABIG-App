    ...
    for col in out.columns:
        try:
            ...
            if out[col].dtype == "object":
                ...
                out[col] = out[col].apply(lambda x: x if (pd.isna(x) or isinstance(x, (str, int, float, bool))) else str(x))
        except Exception:
...
# =========================
# Storage (SQLite + uploads) - use /tmp for Streamlit Cloud safety
# =========================
TMP_DIR = Path("/tmp/asabig")
TMP_DIR.mkdir(parents=True, exist_ok=True)

DB_PATH = TMP_DIR / "asabig.db"
...
def apply_inclusive_gender_filter(df: pd.DataFrame, col: str, selection: str) -> pd.DataFrame:
    """
    Inclusive logic:
    - If selection == M  => show M + M/F (hide F)
    - If selection == F  => show F + M/F (hide M)
    - If selection == M/F => show only M/F
    """
    ...
    with tab1:
        ...
        if st.button("Login"):
            if not email or not password:
                ...
                row = db_fetchone(
                    "SELECT id, email, full_name, role, salt, password_hash FROM users WHERE email = ?",
                    (email.strip().lower(),),
                )
                if not row:
...
# =========================
    ...
    if u["role"] == "Player":
        ...
        with colB:
            ...
            g_list = ["", "M", "F", "M/F"]
            g_val = normalize_gender_value(p["gender"]) if p["gender"] else ""
            gender = st.selectbox("Gender", g_list, index=g_list.index(g_val) if g_val in g_list else 0)
            birth_year = st.number_input(
                "Birth year",
                min_value=1990,
                max_value=2035,
                value=int(p["birth_year"]) if p["birth_year"] else 2010,
            )
            ...
            visibility = st.selectbox(
                ...
                ["Private", "Partial", "Full"],
                index=["Private", "Partial", "Full"].index(p.get("preferences", {}).get("visibility", "Private")),
            )
        ...
        vids = db_fetchall(
            """SELECT title, url, notes, created_at
               FROM videos WHERE player_id=? ORDER BY id DESC LIMIT 20""",
            (player_id,),
        )
        if vids:
            dfv = pd.DataFrame(vids, columns=["Title", "URL", "Notes", "Added"])
            st.dataframe(safe_df(dfv), use_container_width=True)
        else:
    ...
    # -------------------------
        ...
        if rows:
            ...
            if sl:
                ...
                st.dataframe(safe_df(sdf), use_container_width=True)
            else:
...
# =========================
    ...
    st.dataframe(safe_df(pd.DataFrame(users, columns=["ID", "Email", "Name", "Role", "Created"])), use_container_width=True)
    ...
    if docs:
        ...
        st.dataframe(safe_df(ddf), use_container_width=True)
...
- Enables **structured scouting** and **athlete development tracking**
    ...
    st.info("🔬 Pilot demo: DB users/players here are for testing only (synthetic data).")
...
# =========================
    ...
    with st.expander("Summary (numeric columns)"):
        ...
        if numeric_df.empty:
            ...
            st.dataframe(safe_df(numeric_df.describe()), use_container_width=True)
...
- Storage for Pilot uses `/tmp` (Streamlit Cloud safe)  
        """
    )
